<template>
  <div id="top-header">
    ...
    <div class="center-title" ref="title">{{title}}</div>
    ...
    <audio id="cheerMusic" ref="music" preload="auto" src="../../../assets/audio/262.wav" />
  </div>
</template>
...
export default {
    ...
    openFullScreen(id) {
      ...
      let time = setInterval(() => {
        ...
        if (this.isDisba) {
          ...
          _that.loading.background =
            split[0] + ',' + split[1] + ',' + split[2] + ',' + '0.7)'
        } else {
          _that.loading = _that.$loading(_that.option)
          let split = _that.option.background.toString().split(',')
          _that.loading.background =
            split[0] + ',' + split[1] + ',' + split[2] + ',' + '0.5)'
        }
      }, 500)
  ...
  watch: {
    ...
    deptName: function (newValue,lodValue) {
      if(newValue){
        this.title = newValue + '智慧农业管理平台';

        if(this.title.length > 15){
          this.$refs.title.style.fontSize = (450 / this.title.length) + 'px';
        }
      }
    }
  },
